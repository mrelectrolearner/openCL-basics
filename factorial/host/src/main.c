...
int main() {
 ...
 if(error!=CL_SUCCESS) printError(error);
  ...
  // Read FPGA binary
  size_t length=0;
  const char *fileName="factorial.aocx";
  ...
  // Create kernel.
  const char *kernel_name = "factorial";
  ...
  cl_mem dev_dout =
      clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(double), NULL, NULL);
  ...
  printf("work 7\n");
  // Read data from FPGA
  clEnqueueReadBuffer(queue, dev_dout, CL_TRUE, 0, sizeof(double), (void *)(&host_dout),
                      0, NULL, NULL);
  ...
  printf("factorial= %lf, time= %lf",  host_dout, time_taken);
