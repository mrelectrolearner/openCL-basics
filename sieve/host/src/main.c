...
int main() {
 ...
 if(error!=CL_SUCCESS) printError(error);
  ...
  // Read FPGA binary
  size_t length=0;
  const char *fileName="seive.aocx";
  ...
  int  *host_dout;
  host_din_A=10000000;
  host_dout=(int *)malloc((host_din_A + 1) * sizeof(int));
  //posix_memalign((void **)(&host_dout), 2, sizeof(int) * (host_din_A+1));

  for (int i = 0; i <= host_din_A; i++) {
    host_dout[i] = 0;
  }
  ...
  cl_mem dev_dout =
      clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int)*(host_din_A+1), NULL, NULL);

  printf("work 5\n");
  // FPGA side data
  clEnqueueWriteBuffer(queue, dev_din_A, CL_TRUE, 0, sizeof(int) ,
                       (void *)(&host_din_A), 0, NULL, NULL);


  printf("work 6\n");

  // Execute kernel
  clSetKernelArg(kernel, 0, sizeof(cl_mem), &dev_din_A);
  ...
  printf("work 7\n");
  // Read data from FPGA
  clEnqueueReadBuffer(queue, dev_dout, CL_TRUE, 0, sizeof(int)*(host_din_A+1), host_dout,
                      0, NULL, NULL);
  ...
  printf("seive(%d)execution time= %Lf secs\n", host_din_A,time_taken);
